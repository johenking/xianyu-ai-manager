...
The account QR panel offers two explicit choices and opening it starts neither. “Local Chrome QR” creates `{"mode":"qr","show_browser":true}` and opens on the service Mac; “Web QR” keeps the existing `/qr-login/*` path for remote access, and ordinary web generation/scanning does not launch a browser. SMS login, explicit password login, local Chrome QR, web-QR secondary verification, and password renewal use installed system Chrome in headed mode because Goofish rejects headless Chromium. Switching modes or closing the panel must cancel an active local Chrome session; a web QR session is left to its existing TTL cleanup. Automatic renewal is available only to password accounts with valid stored credentials. The current container entrypoint does not provide system Chrome or a virtual display, so do not claim Docker or cloud login renewal works until the browser, display/Xvfb, and human-verification workflow have been tested on that deployment.
...
4. For a server-browser session, wait for `verification_required` with `browser_active=true`, then use “open on this Mac” from an administrator loopback console and complete login in that same official window.
5. For remote web QR, keep mobile-scan verification as a scannable image and move slider, face, or unknown interactive risk control to the user's protocol-v2 Chrome extension. Hiding the modal must not stop polling.
...
7. Use administrator-only local Chrome QR or SMS on the service Mac, web QR for remote access, explicit password login, the user's Chrome extension, or a matching manual Cookie when the saved profile cannot recover the session. Never update an existing record with a different Cookie `unb`; the API must return HTTP 409 `account_identity_mismatch`.
