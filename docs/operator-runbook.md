...
5. For remote web QR, keep mobile-scan verification as a scannable image. Keep slider, face, SMS, or unknown interactive risk control in the same server-Chrome session and operate it through the owner-scoped live console surface. Hiding the modal must not stop polling; the Chrome extension is an explicit advanced import option only.
...
7. Use administrator-visible server Chrome on the service Mac, remote web QR or SMS through the live interaction surface, explicit password login, the optional user Chrome extension, or a matching manual Cookie when the saved profile cannot recover the session. Never update an existing record with a different Cookie `unb`; the API must return HTTP 409 `account_identity_mismatch`.
