...
Official QR login, explicit password login, and renewal launch the installed system Chrome in headed mode because Goofish rejects headless Chromium. Automatic renewal is profile-only. The current container entrypoint does not provide system Chrome or a virtual display, so do not claim Docker or cloud login renewal works until the browser, display/Xvfb, and human-verification workflow have been tested on that deployment.
...
Symptoms include missing message Tokens, expired Cookies, passive `action_required`, or active `verification_required`.
...
3. Keep the account listener running. In `action_required`, trigger `/session-refresh` exactly once; repeated Token or connection failures must not create a browser.
