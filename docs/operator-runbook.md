...
The account QR panel recommends “Web QR” and offers “Server Chrome” as a fallback; opening the panel starts neither. Web QR keeps `/qr-login/*` independent and ordinary generation/scanning does not launch a browser. The server-browser fallback requires an authenticated console session and is exposed by the UI only on loopback and the formal production hostname. Loopback sends `show_browser=true` and keeps the visible local-window flow; `xianyu.cxywjx.top` sends `show_browser=false`, displays the live cloud Chromium frame in the page, and reuses the existing click, drag, wheel, text, and key interaction endpoints. Unfamiliar hostnames continue through the extension. Switching modes or explicitly cancelling ends the matching session; hiding the panel does not stop polling.

The container entrypoint starts a background `Xvfb :99`, exports `DISPLAY`, then `exec python Start.py` so every headed login session receives a real `DISPLAY` while Python stays PID 1 (never use `xvfb-run` as PID 1: it deadlocks waiting for the X readiness signal and does not forward SIGTERM). Shared browser launch policy keeps the Chromium sandbox for non-root processes and disables it only when the application process is root; an unset `XIANYU_BROWSER_CHANNEL` means Playwright bundled Chromium on official login, L3 memory, and QR secondary verification. `profile_in_use` is reserved for `ProcessSingleton`/`SingletonLock`; sandbox, missing-display, and generic profile errors are `browser_error`.

When copying `browser_data/` between hosts, exclude `SingletonLock`, `SingletonCookie`, and `SingletonSocket`. To repair a migrated volume, stop only APP_A, confirm there is no Chrome/Chromium process, create a mode-0700 private archive of those lock entries, remove only the three lock names, and then recreate APP_A. Never remove Cookie, Preferences, profile directories, account data, volumes, configuration, or encryption keys. A rollback may restore archived locks only while APP_A is stopped and only if no new Singleton lock exists.
