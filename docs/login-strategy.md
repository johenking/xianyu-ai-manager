...
> **适用前提**：本策略面向单机自用形态——生产服务作为 LaunchAgent 运行在用户本机 Mac（本地 8091）。控制台可从回环或正式域名访问；前端只在回环与正式域名展示服务端浏览器备选入口，其他域名保留网页二维码、浏览器扩展和手动导入。

## 最终结论

网页二维码是当前 UI 推荐主路径：只请求官方二维码并在控制台渲染，零安装且不会启动浏览器。服务端本机官方窗口是备选路径；服务在这台 Mac 上用 Playwright 打开闲鱼官方登录页，未配置 `XIANYU_BROWSER_CHANNEL` 时使用 Playwright 自带的 chromium，配置为 `chrome`/`msedge` 等值时沿用对应系统浏览器渠道。窗口使用隔离 Profile，不复用用户日常浏览器。
...
|---|---|---:|---|---|
| `qr` | 默认使用独立网页二维码；回环或正式域名可选服务端官方窗口备选 | 扫码成功写入持久 profile 后可免密续签 | 记忆失效时重新扫码 | 网页二维码遇交互风控转官方窗口或扩展；普通扫码不启动浏览器 |
| `sms_window` | 服务端官方窗口在本机完成手机号验证码登录；远程设备使用扩展 | 否 | 重新登录 | 验证码只留在浏览器窗口，不进入控制台 |
| `password` | 服务端官方窗口在本机完成账号密码登录；远程设备使用扩展 | 否 | 重新登录 | 密码、短信、人脸和页面交互不进入控制台 |
