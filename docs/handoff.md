...
The v1.7.3 release-candidate source keeps the v1.7.2 authentication experience while preventing duplicate official Cookie-refresh sessions and bringing Skill Center plus administrator and ordinary-user settings into the protected application visual system. Manual refresh now reserves an account before scheduling work, listener restarts restore persisted refresh timing, and Cookie-driven restarts defer item synchronization until its configured interval. Passwords, Cookies, email codes, reset grants, API keys, deployment tokens, databases, browser profiles, and live account data remain outside source control.

Local v1.7.3 release gates are recorded below after they run. GitHub CI and the running service are independent evidence: publishing or building this source does not prove a runtime was upgraded. v1.7.3 adds no database migration; the latest expected migration remains `2026071104`. Verify the process path, health response, migration version, frontend entry bundle and referenced asset, account listeners, disabled Cookie schedules, and Skill scheduler before describing any deployment as running v1.7.3. Registration defaults closed on a new installation and must not be opened until the real SMTP receipt code and an end-to-end direct-registration acceptance test have both succeeded.
...
- One official refresh path for manual refresh, scheduled refresh, Token expiry, and repeated connection failures, with account-page secondary verification and cancellation. Active manual requests do not queue, and listener restarts inherit persisted renewal and item-sync timing.
...
Verified locally on 2026-07-12 for the v1.7.3 release candidate: Ruff and explicit Python compilation passed, all 229 backend tests passed, and all 17 frontend test files with 67 tests passed. Gitleaks found no leaks in Git history or the working directory, `npm audit --audit-level=high` reported zero vulnerabilities, and `git diff --check` passed. Two final production builds retained two generations of 30 assets with zero orphans; the 245,154-byte entry bundle is 71.7% below the 865,910-byte baseline, while the 402,730-byte chart bundle remains deferred. The authenticated 1440x900 and 390x844 visual checks and the two-minute live manual-refresh observation remain deployment acceptance steps; they are not inferred from unit tests.
...
- Deploy the exact v1.7.3 source and clean static build only after backup, then verify migration remains `2026071104`, the Skill Center and both settings roles match the protected application UI at 1440x900 and 390x844, and authentication logs contain none of the prohibited values.
- Keep both account Cookie schedules disabled, manually refresh one account once, and observe for at least two minutes. Require one official session, one listener restart, no later scheduled refresh, and no immediate item-detail Chrome for Testing process.
