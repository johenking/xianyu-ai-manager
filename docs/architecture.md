...
The packaged native helper has been removed. On the single-machine deployment the loopback console drives a server-side headed Playwright Chromium that opens the official login page directly on this Mac, so local login requires no client install. The browser extension remains the only user-machine channel; it submits signed login state directly to `/api/client-browser/import`, and Cookie, Token, password, OTP, and verification content still do not pass through the frontend.
