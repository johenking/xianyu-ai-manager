...
`schema_migrations` records ordered migrations. A pending migration backs up the SQLite database plus the AI-provider, Xianyu-account, and system-secret local keys before starting one transaction. The compatibility upgrader keeps database version `1.6` idempotent while ordered migrations add the registration security schema, normalized identity indexes, `2026071104` order-analysis indexes, `2026071701` persisted official-browser identity, `2026071801` durable monitor workflow state, and the expand-only `2026071802` MTop budget/breaker safety tables. A case-insensitive identity conflict aborts migration instead of merging users. Xianyu login passwords use an account-specific Fernet key that is separate from the AI provider and system-secret keys; account detail and status APIs never return the plaintext or ciphertext.
...
`utils/xianyu_official_login.py` owns official browser authentication. New QR and explicit password sessions open the parent `https://www.goofish.com/login` page with the installed system Chrome in headed mode. Background work only positions that window off-screen; the service does not override the browser User-Agent, inject anti-detection behavior, disable web security, or use a headless verification browser. New sessions use a temporary `.login_<uuid>` profile. Cookie presence is only a prerequisite: success also requires a real message `accessToken` from the shared session probe, expected-`unb` matching, and no active login or security surface. The browser's actual User-Agent is persisted with the Cookie and reused by Token and listener traffic. The validated browser remains open while Cookie persistence and the single listener replacement complete, then the temporary profile is atomically promoted to `browser_data/user_<unb>` while preserving the previous profile on promotion failure.

Cookie refresh state is persisted in `account_session_refresh_status` with `idle`, passive `action_required`, active `refreshing` and `verification_required`, plus `success`, `failed`, `timeout`, and `cancelled`. Token or repeated connection failures only write `action_required`; they do not create a browser or restart the listener. A browser may start once after the user clicks the manual action, or once when an enabled account schedule becomes genuinely due. Manual requests atomically reserve the account before background scheduling, and duplicate requests return the active state without queuing. Every failure, cancellation, or timeout anchors the next scheduled attempt to a complete configured interval. Listener replacement receives the already-probed Token, persisted browser User-Agent, renewal anchor, and item-sync anchor in one generation, so QR completion does not immediately issue another Token request. On process startup, orphaned active states are normalized to `action_required` because no browser Worker survived the restart.
...
`official_login_sessions.py` owns the unified new-account QR/password state machine: `preparing`, `waiting_user`, `verification_required`, `persisting`, `restarting_listener`, and the terminal states `success`, `expired`, `failed`, `cancelled`, or `interrupted`. One background task performs completion exactly once; status reads have no lifecycle side effects. Temporary login, AI training, and Cookie refresh operations share a Session Registry. `runtime_sessions` stores only session type, owner, account identifier, state, redacted error, and TTL. Polling another user's session returns HTTP 403. The password-login task receives credentials only as background-task arguments; the password is not stored in login dictionaries or the registry. Cookies, Tokens, verification URLs, Playwright objects, and AI conversation content are also excluded. On restart, active browser-backed records become `interrupted` and the UI must ask the operator to start again.
...
- `skill_monitor_runs`, `skill_monitor_events`, `skill_monitor_result_identities`, `skill_monitor_deliveries`: lease-owned execution, first-seen/AI/delivery events, stable result identity, and transactional outbox state.
- `skill_monitor_request_budgets`: digest-only fixed-window account/global MTop request counters; rows expire one day after the window closes.
- `skill_monitor_mtop_breakers`: digest-only consecutive-failure state plus one leased half-open probe; no account ID or error text is retained.
...
Stage C adds a separate MTop adapter but does not route monitor runs to it. Each request/retry requires the global switch, the MTop DB switch, and `SKILL_MONITOR_MTOP_NETWORK_ALLOWED=true`; all default off. The endpoint is fixed, redirects/arbitrary URLs are rejected, risk control stops without account rotation, and a gate change discards the response before Cookie CAS. Account/global request budgets are claimed atomically in SQLite. A digest-only circuit opens after three consecutive remote/protocol failures, opens immediately for risk/action-required states, and single-flights one leased half-open probe after cooldown. Response size/schema are bounded, only normalized item fields leave the adapter, and refreshed Cookies use the existing owner/account/`xianyu_unb`/revision CAS. Pagination, price/region filtering, latest/price sorting, `Retry-After`, jitter, run-time/page/result caps, and safe error codes are deterministic and fake-transport testable.

The shadow comparator accepts only normalized allowlisted fields and reports recall versus Playwright, Jaccard overlap, price/region mismatch ratios, rank displacement, and explicit threshold failures. Both-empty results count only for a query declared legally empty; an expected-non-empty canary cannot pass with zero results. Monitor AI output is an exact three-field JSON contract and is rejected on timeout, refusal/non-JSON, extra fields, or lost run lease. Full responses and provider text are not audit payloads.
...
The capability API and Skill Center UI keep six claims separate: code present, configuration ready, last successful real search, last scheduled run, last persisted AI decision, and last confirmed real delivery. Each runtime claim is backed by a user-scoped run, event, or delivery row and timestamp; absent evidence is shown as never verified. `code_present.evidence.offline_mtop_adapter` is a backward-compatible code/configuration detail, not a seventh runtime capability; it exposes the disabled gates, absolute limits, unverified canary, and dedicated-test-account blocker.

Monitor runs, results, first-seen/AI events, and deliveries use the existing 30-day `retention_until` policy. Budget rows expire one day after their window. Logs retain only safe error codes/classes and masked identifiers; Cookies, signatures, full responses, raw AI text, and destination secrets are excluded.
