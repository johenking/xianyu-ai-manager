...
Direct registration is fail-closed. `registration_enabled` defaults to false and migration `2026071103` forces it false, sets agreement `v2`, installs a default ordinary-user limit of 20, and consumes invitation-era registration challenges. Readiness requires a valid support email, receipt-code confirmation for the exact current SMTP fingerprint, and remaining ordinary-user capacity. Disabled ordinary users count toward capacity; the administrator does not. A `BEGIN IMMEDIATE` registration transaction rechecks the switch and capacity, validates the purpose-bound email challenge, normalized identities, password policy, and terms version, then creates the user and consumes the challenge. When the administrator has enabled `registration_invite_required`, the same transaction also requires a valid invite code and consumes it exactly once. Filling the last slot closes the switch in the same transaction.

Invite codes are a live runtime feature again: `registration_invites` stores an HMAC digest plus a display hint, optional note, expiry, revocation, and consumption state; the plaintext code is returned exactly once at creation. The administrator toggles enforcement through the `registration_invite_required` setting, which defaults off so existing open-registration deployments keep their semantics. `auth_challenges` stores digests for image CAPTCHA, registration email, password-reset email, password-reset grant, and SMTP receipt-code secrets, with expiry, attempt, and consumption state. `auth_rate_events` stores HMAC digests for IP, email, and account dimensions. Forwarded client addresses are considered only when the direct peer is in the `auth_trusted_proxies` setting.
...
`ai_provider_profiles` stores user-scoped provider profiles, encrypted API keys, the default model, cached model lists, and verification state. OpenAI-compatible providers use Chat Completions and `/models`; Gemini uses its native models list and `generateContent`. Accounts bind to a profile and select their own model. A provider/model change must generate a successful test reply before it can replace the active account configuration. At reply time, key resolution falls back in this order: the account-bound profile, the account's own key, the administrator's default verified profile (site-level sharing for agent users), then the system-wide key. A site-level fallback is surfaced as `api_key_source: "site"` with `provider_profile_id: null`, never exposes the administrator's plaintext key or profile ownership to the agent, and an unverified administrator profile is never lent out.
...
仪表盘首先调用一个汇总接口。个人仪表盘对所有角色（含管理员）都只读取通过自己 `user_id` 所有者 `cookies` 关联出的数据，接口报告 `scope: "user"`。站级视图与之分离：`GET /api/dashboard/global-summary` 对任何登录用户返回 `scope: "site"` 的全站经营合计（只含站级总数，绝不下发分用户明细）；`GET /api/admin/dashboard/agents`（分代理明细）与 `GET /api/admin/accounts/overview`（账号总览与归属）为管理员独享。管理员权限另覆盖注册、用户和系统管理。响应包含计数、当前与上一周期分析、商品名称和显式 `trend_granularity`：单日范围携带稀疏的东八区 `hourly_stats`，多日范围使用 `daily_stats`。订单明细在汇总渲染后再加载，Recharts 依旧位于独立懒加载 chunk。分析查询使用时间边界而不是对 `created_at` 包裹 `DATE()`，以便 SQLite 使用迁移索引。
...
- `registration_invites`, `auth_challenges`, `auth_rate_events`: digest-based runtime invite codes with expiry/revocation/consumption state, purpose-bound authentication and SMTP challenges, and persistent rate-limit events stored as digests.
...
- Registration admin: `/api/admin/registration/status`, `/limit`, `/users`, `/enabled`, invite management (`GET`/`POST /api/admin/registration/invites`, `DELETE /api/admin/registration/invites/{invite_id}`), and `PUT /api/admin/registration/invite-required`; ordinary users can be enabled or disabled without destructive deletion.
...
- Orders and analytics: role-aware `GET /api/dashboard/summary`, site-total `GET /api/dashboard/global-summary`, admin-only `GET /api/admin/dashboard/agents` and `GET /api/admin/accounts/overview`, structured `POST /api/orders/sync`, `/api/orders*`, order timing and buyer behavior, `/analytics/items/performance`, `/analytics/items/traffic`, and account-scoped metric status/manual canaries.
...
`item_info.delivery_mode` is the current source of truth for explicit product delivery: `off`, `resource`, or `invite`. Legacy `/items/{cookie_id}/{item_id}/delivery-binding` remains readable for older clients; its `card_id` binding maps to `resource`, while `null` maps to explicit `off`, not keyword fallback. Keyword delivery rules remain a compatibility fallback only while a newly synchronized item has no explicit mode. Within that keyword path, a non-administrator user whose own rules produce no match falls back to matching the administrator's keyword rules and consumes the administrator's card inventory; the user's own rules always win first, explicit delivery modes never trigger this fallback, and delivery counters land on the administrator rule row that actually matched. Product delivery and `invite_auto_fulfillment` are mutually exclusive, so enabling either clears the other instead of allowing two inventory owners.
