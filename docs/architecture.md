...
`native_browser_helper/` is the ordinary-user browser boundary. A packaged helper runs on the user's macOS or Windows computer, binds only `127.0.0.1:17890`, accepts allowlisted console Origins, and stores its P-256 device key in macOS Keychain or Windows DPAPI. The frontend reads only the helper's public device record and safe status. The helper opens a Chrome/Edge target through CDP, watches official-page and Cookie changes locally, and submits the signed login state directly to `/api/client-browser/import`; Cookie, Token, password, OTP, and verification content do not pass through the frontend.

`client_browser_login.py` owns the five-minute device/session/challenge protocol. Migration `2026080101` adds `client_browser_devices.client_type`; `native_helper` and `extension` devices cannot consume each other's sessions or renewal tasks. A login reaches success only after the server validates a real message Token and `unb`, persists the account, and the frontend confirms the account list. The helper or extension then closes only its own official tab. Temporary probe or persistence failures release the import for a fresh challenge instead of turning into a human-verification state.

`utils/qr_login.py` owns the independent API QR path and renders the official `codeContent` locally; ordinary QR generation and scanning do not launch a browser. Secondary interactive verification is handed to the user-machine helper or extension. `utils/xianyu_official_login.py`, `utils/qr_verification_browser.py`, and `utils/browser_interaction.py` remain server-side facilities for administrator loopback maintenance and legacy password renewal, not ordinary-user browser login. Those flows use a normal installed system Chrome, do not override its User-Agent or disable web security, and still require a real message `accessToken`, expected-`unb` matching, Cookie persistence, and listener replacement before success.
...
Order item images resolve through a two-step chain. The client first requests the authenticated media endpoint `GET /api/orders/{order_id}/item-image`, which serves an application-side cached copy of the image bound to that order and only fetches from a trusted host list over pinned, publicly routable addresses. When that endpoint fails for any reason and the order still carries a CDN link, the client falls back to loading that link directly in the browser with a no-referrer policy. The failure placeholder with its retry control appears only when no image source exists at all or the direct link itself fails to load, so a server-side fetch restriction no longer hides an image the order actually has.
...
The dashboard first calls one summary endpoint. Every role, administrators included, receives only rows joined through its own `user_id`-owned `cookies`; the endpoint always reports `scope: "user"` and there is no system-scope dashboard. Administrator privileges cover registration, user, and system management instead. The response combines counters, current and previous analytics periods, and product names. Order detail loading starts after the summary renders, and the Recharts dependency lives in a separate lazy chunk. Analytics use timestamp boundaries instead of wrapping `created_at` in `DATE()` so SQLite can use the migration indexes.

The dashboard surface is a dark cockpit hero followed by light analysis panels. The hero carries the headline revenue figure, a period-over-period badge that stays neutral when the previous period is zero, a revenue trend that is zero-filled across the selected range so days without orders render as zero instead of being skipped, and secondary counters. `frontend/components/ui/dashboardParts.tsx` holds the shared vocabulary used by `Dashboard.tsx`, `DashboardCharts.tsx`, and `BusinessInsights.tsx`: panel and title primitives, a low-key inline note, horizontal share bars that replace pie charts, HTML rank lists that replace truncating vertical bar axes, one semantic order-status color map, an unnamed-product fallback label, and shared money and count formatters.
...
- `client_browser_devices`, `account_renewal_bindings`, `client_renewal_tasks`: typed native-helper/extension public devices, extension-only renewal authorization, and one-time encrypted renewal delivery.
...
- Account binding: ordinary users use `/api/client-browser/devices`, `/api/client-browser/sessions`, `/api/client-browser/challenges`, `/api/client-browser/import`, and session `confirm`/`cancel`; the native helper exposes its separate loopback `/health`, `/v1/device`, and `/v1/login/*` surface. `/api/official-login/sessions` and session `show-browser`/`cancel` are administrator-loopback maintenance only. Compatibility `/qr-login/*`, `/password-login*`, and `/cookies*` remain available; caller-supplied account IDs are not authoritative.
...
The native helper is a separate per-user deliverable. macOS and Windows packages must pass install/startup/health/single-instance/restart/status/uninstall lifecycle checks on native runners. Public macOS distribution still requires a Developer ID Application signature and notarization; Windows requires Authenticode. Package availability does not replace an ordinary-user-machine canary through real platform verification, persistence, frontend confirmation, and helper-tab closure.
