...
© 2025 闲鱼自动回复系统"""
    ...
    def get_skill_monitor_task(self, task_id: int, user_id: int) -> Optional[Dict[str, Any]]:
        with self.lock:
            try:
                cursor = self.conn.cursor()
                cursor.execute('''
                ...
                ''', (task_id, user_id))
                row = cursor.fetchone()
                return self._skill_monitor_task_from_row(row) if row else None
            except Exception as e:
                logger.error(f"获取技能监控任务详情失败: {e}")
                return None

    def create_skill_monitor_task(self, user_id: int, task_data: Dict[str, Any]) -> Optional[int]:
        with self.lock:
            try:
                cursor = self.conn.cursor()
                cursor.execute('''
                ...
                ''', (
                    ...
                    task_data.get('min_price'),
                    task_data.get('max_price'),
                    task_data.get('region', ''),
                    task_data.get('published_within_hours', 24),
                    task_data.get('ai_filter', ''),
    ...
    def update_skill_monitor_task_run(self, task_id: int, user_id: int) -> bool:
        with self.lock:
            try:
                ...
                logger.error(f"更新技能监控任务运行时间失败: {e}")
                self.conn.rollback()
                return False

    def create_skill_monitor_result(self, result_data: Dict[str, Any]) -> Optional[int]:
        with self.lock:
            try:
                cursor = self.conn.cursor()
                cursor.execute('''
                    INSERT INTO skill_monitor_results (
                        task_id, user_id, title, price, region, item_url, item_image,
                        seller_name, ai_score, ai_reason, notify_status, raw_data
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    ...
                    result_data.get('item_url', ''),
                    result_data.get('item_image', ''),
                    result_data.get('seller_name', ''),
                    result_data.get('ai_score', 0),
                    result_data.get('ai_reason', ''),
                    result_data.get('notify_status', 'pending'),
                    json.dumps(result_data.get('raw_data', {}), ensure_ascii=False),
                ))
                self.conn.commit()
                return cursor.lastrowid
            except Exception as e:
    ...
    def list_skill_monitor_results(self, user_id: int, task_id: int = None, limit: int = 100) -> List[Dict[str, Any]]:
        with self.lock:
            try:
                ...
                params.append(limit)
                cursor.execute(f'''
                    SELECT id, task_id, user_id, title, price, region, item_url, item_image,
                           seller_name, ai_score, ai_reason, notify_status, raw_data, created_at
                    FROM skill_monitor_results
                    WHERE {' AND '.join(conditions)}
                    ORDER BY created_at DESC, id DESC
                    LIMIT ?
                ''', params)
                return [self._skill_monitor_result_from_row(row) for row in cursor.fetchall()]
            except Exception as e:
                logger.error(f"获取技能监控结果失败: {e}")
                return []
    ...
    def log_skill_event(self, user_id: int, module: str, message: str, level: str = 'info', payload: Dict[str, Any] = None) -> bool:
        with self.lock:
            try:
                cursor = self.conn.cursor()
                cursor.execute('''
                    INSERT INTO skill_run_logs (user_id, module, level, message, payload)
                    VALUES (?, ?, ?, ?, ?)
                ''', (
                    user_id,
                    module,
                ...
                logger.error(f"记录技能运行日志失败: {e}")
                self.conn.rollback()
                return False

    def list_skill_logs(self, user_id: int, module: str = None, limit: int = 50) -> List[Dict[str, Any]]:
        with self.lock:
            try:
                cursor = self.conn.cursor()
                conditions = ["user_id = ?"]
                ...
                params.append(limit)
                cursor.execute(f'''
                    SELECT id, module, level, message, payload, created_at
                    FROM skill_run_logs
                    WHERE {' AND '.join(conditions)}
                    ORDER BY created_at DESC, id DESC
                    LIMIT ?
