...
该扩展是独立的浏览器导入与续期路径，支持 Chrome 和 Edge；普通用户的“本机 Chrome 登录”主路径由本机助手承接，不依赖扩展检测。用户在自己的浏览器中完成
...
4. 打开咸鱼监控台并保持登录，在独立的“浏览器扩展导入”入口启动扩展流程；本机助手和网页二维码入口不会检测本扩展。
5. 如果使用手动配对导入，在“添加账号 → 高级与运维方式 → 你的 Chrome”创建配对，打开并登录闲鱼官网，点击扩展图标，粘贴配对信息后主动导入。
