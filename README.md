...
- 多账号管理：当前设备 Chrome/Edge 登录、网页二维码、官方手机号验证码、账号密码、管理员服务器运维登录、用户扩展导入和手动 Cookie 入口，按真实 `unb` 保留账号数据；自动续期只在登录后由用户明确授权并绑定当前设备时启用。
...
- 经营仪表盘：首屏一次读取当前用户的账号、订单、库存和营收汇总，独立加载订单时段、成交爆品、客户行为和商品表现；管理员查看系统范围。真实商品流量只有在卖家后台指标适配器完成账号级三次金丝雀后才展示，默认关闭。
...
新增账号的扫码面板先显示两个并列入口，打开面板本身不会发起登录请求：

- **当前设备浏览器登录**：通过 Chrome/Edge 扩展桥接，在用户正在使用的浏览器中打开官方页面。扫码、手机号、账号密码以及滑块、人脸、短信或未知交互验证都由用户在自己的浏览器完成；普通用户路径在服务端启动的 Chrome 数量为 0。
- **网页二维码**：调用官方接口取得 `codeContent` 并在本地渲染，适合手机扫码。若平台要求继续风控，页面明确提示回到当前设备浏览器完成验证，不启动服务器浏览器。

扩展只转发当前标签页的登录 Cookie、User-Agent 和一次性设备证明；服务端仍会真实验证平台 Token、`unb` 身份和账号落库。只有前端再次确认账号出现在账号列表后，扩展才关闭官方标签页。扩展缺失时不创建服务端会话，界面提供安装/刷新和网页二维码回退。
...
当前设备 SMS/密码登录不会在控制台收集手机号、密码或验证码。密码登录成功后，用户可以另行填写并明确授权保存续期凭据；凭据使用独立密钥加密，并只绑定一个当前设备。续期任务通过 60 秒一次性加密交付发送到该设备，首次领取后服务端立即清空密文，不能重复领取。
