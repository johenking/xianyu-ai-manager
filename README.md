...
- 邀请注册：单次邀请码、图形验证码和邮箱验证码在同一事务内完成注册；支持用户名或邮箱登录、密码找回、协议页面和普通用户启停。
...
公开注册默认关闭。管理员需要先在“系统与 AI”中保存 SMTP 与支持邮箱，并通过一封真实测试邮件验证当前配置，再创建至少一个有效的单次邀请码，之后才能开启注册。SMTP 配置、指纹或授权码发生变化时，验证状态会立即失效，注册自动保持关闭。

注册页面使用一次性图形验证码和用途隔离的邮件验证码。邀请码、验证码和网络标识在数据库中只保存 HMAC 摘要；用户名和邮箱按规范化值检查唯一性。注册成功后直接创建摘要 Session 并登录，任一步失败都会回滚用户、邀请码和验证码消费。
...
SMTP 授权码、邀请码/验证码摘要和脱敏网络标识使用第三把系统秘密。生产环境建议设置 `SYSTEM_SECRET_ENCRYPTION_KEY`；未设置时会生成 `data/.system_secret_key`。这三把本地密钥必须与 SQLite 一起备份，缺失任何一把都可能导致相应密文或摘要无法继续使用。
...
- SMTP 对核心闲鱼自动化仍是可选能力，但邀请注册和密码找回依赖它。SMTP 验证会向支持邮箱或发件邮箱发送真实测试邮件；没有成功送达时不能开放注册。
