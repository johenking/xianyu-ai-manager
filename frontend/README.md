...
Opening the account dialog creates no login session. Web QR is the primary path; after a scan, mobile-scan verification remains a scannable image while slider, face, SMS, and unknown interactive verification continue in the same server-Chrome session through an owner-scoped live interaction surface. Remote SMS uses the same interaction surface. Closing or hiding the dialog does not turn a scanned QR or a browser page closing into success: polling continues until a real platform Token is validated and the account identity and Cookie are persisted, or until the user explicitly cancels or changes methods. Physical server-window controls remain limited to an administrator on the loopback console. The Chrome extension is an explicit advanced import option, and password and manual Cookie remain secondary methods. Account renewal reuses the official browser profile and reads saved credentials only after the profile is fully logged out.
