...
class XianyuOfficialLoginService:
        ...
        *,
        ...
        for launch_number in range(2):
            ...
            try:
                ...
                entry_url = (
                    active_verification_url
                    or (GOOFISH_LOGIN_URL if mode in {"qr", "password"} else GOOFISH_IM_URL)
                )
                ...
                if self._has_active_login_form(page):
                    ...
                    elif mode == "refresh" or not allow_password:
                ...
                monitor_result = self._monitor_login(
                    ...
                    on_status=on_status,
                    verification_image_path=verification_image_path,
                    wait_for_login_form=mode in {"qr", "refresh"},
                ...
                return OfficialLoginResult(
                    ...
                    message=f"官方浏览器会话失败: {sanitize_runtime_error(message)}",
                    used_password=used_password,
                )
            finally:
    ...
    def _monitor_login(
        ...
        *,
        ...
        login_deadline = time.monotonic() + (
            self.verification_timeout if wait_for_login_form else self.login_timeout
        )
    ...
    def _apply_validated_hook(
        ...
        try:
            ...
            return OfficialLoginResult(
                ...
                message=f"消息 Token 已验证，但保存或监听交接失败: {sanitize_runtime_error(exc)}",
                used_password=result.used_password,
