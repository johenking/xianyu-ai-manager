...
class XianyuApis:
    def __init__(self):
        self.url = _resolve_h5_api_url('https://h5api.m.goofish.com/h5/mtop.taobao.idlemessage.pc.login.token/1.0/')
    ...
    def get_token(self, device_id, retry_count=0):
        ...
        try:
            response = self.session.post(_resolve_h5_api_url('https://h5api.m.goofish.com/h5/mtop.taobao.idlemessage.pc.login.token/1.0/'), params=params, data=data)
            res_json = response.json()
...
# 示例调用请通过环境变量传入 Cookie，源码不保存任何登录凭据。
