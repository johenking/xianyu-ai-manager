"""Side-effect-free message-token probing for official Xianyu sessions.

The browser/login layer and the long-running listener use this module as the
single source of truth for deciding whether a Cookie can really establish the
message session.  Verification URLs stay in memory and are never logged or
...
def detect_default_browser_user_agent() -> str:
    """Build a Chrome UA from the installed browser when no persisted UA exists."""
    ...
    for plist_path in candidates:
        try:
            ...
            os_token = mac_version.replace(".", "_")
            return (
...
def is_allowed_verification_url(value: str) -> bool:
    ...
    if parsed.scheme != "https" or not host:
        return False
    return any(host == allowed or host.endswith(f".{allowed}") for allowed in _ALLOWED_VERIFICATION_HOSTS)
