...
class OfficialLoginSessionCoordinator:
    ...
    def _flight_key(owner_user_id: int, mode: str, account: str) -> str:
        account_key = account.strip().lower() if mode == "password" else "official-qr"
    ...
    async def start(
        ...
        *,
        ...
        if mode not in {"qr", "password"}:
            raise ValueError("登录方式仅支持 qr 或 password")
        ...
        record.task = asyncio.create_task(
            self._run(record, password=password, show_browser=show_browser),
        ...
        try:
            ...
            if record.state not in TERMINAL_STATES:
                self._set_state(
                    record,
                    "failed",
                    f"官方登录任务异常: {sanitize_runtime_error(exc)}",
    ...
    def _set_state(
        ...
        *,
        ...
        self.registry.update(
            ...
            error_message=message if state in TERMINAL_STATES else "",
