...
- Add one owner-scoped official-login session API for QR and explicit password modes, with read-only polling, browser display, cancellation, expiry, interruption recovery, and a unified state machine.
- Add a shared real-message-Token probe, passive `action_required` refresh state, live `browser_active` status, and migration `2026071701` for the official browser User-Agent.

### Changed

- Open the official parent login page in the installed system Chrome, keep background sessions headed and off-screen, and promote successful temporary profiles by the authenticated `unb` identity.
- Restrict browser renewal to an explicit manual action or one genuinely due scheduled attempt; Token and connection failures now pause in `action_required` without launching a browser.
- Delegate the existing QR, password, and account-refresh endpoints to the official session service, and replace account listeners with bounded cancellation outside the account lock.
- Keep one human-verification window open for up to 15 minutes and close it only after real Token validation, Cookie/User-Agent persistence, and one listener handoff complete.

### Fixed

- Remove legacy diagnostics and account-edit guidance that claimed automatic renewal depends on a saved password; renewal now consistently describes the profile-only flow.
- Mask account identifiers in listener-bootstrap logs before the lazily imported listener module installs the global log patcher.

### Security

- Remove automatic slider handling, headless QR verification, User-Agent overrides, anti-detection injection, and public verification-URL exposure from active login paths.
- Redact Cookie values, Tokens, passwords, verification URLs, and sensitive error content from runtime session state, and mask stable account identifiers in operational logs.
