...
- Add the `2026082401` delivery center: three owner-scoped views, fixed资料/一次一密/图片/幂等 API v1 resources, TXT/CSV stock replenishment, encrypted API Tokens, atomic `off`/`resource`/`invite` item modes, masked fulfillment history, and committed-payload resend.
- Add product-level Auto Delivery bindings with keyword-rule fallback and mutual
  exclusion from invitation fulfillment.
- Add migration `2026081601` and a default-off, account-owned seller auto-rate
  flow for orders created after enablement with the platform `RATE` action.
  Reviews are delayed 5-15 minutes, generated by the configured AI with a fixed
  positive fallback, and submitted serially.
- 增加实时仪表盘趋势契约：单日按东八区小时下单时间桶返回，多日按天返回；前端补齐缺失小时，使整天始终显示 24 个稳定点。

### Fixed

- Keep the long-lived account WebSocket listener registered during full and paginated product sync by constructing temporary `XianyuLive` clients with `register_instance=False`.
- Remove account-sync and global-discovery queueing from the paid invitation
  fast path by preferring numeric single-order detail and scanning only the
  already verified target order. Keep the bounded list fallback, independent
  bridge payment verification, idempotency, and message-ACK gate.
- Use the merchant rating contract's `tradeIdList` request field and classify
  success only from `data.module.success` plus `successOrderIds`; inconclusive
  writes persist as `needs_reconcile` instead of being replayed.
- Restore the maintained frontend identity in production: Auto Delivery present,
  Skill Center absent, and native-helper/“本机助手” UI wording absent.
...
- Retire the Skill Center page, scheduler, and `/api/skills/*` routes while
  retaining historical tables and the AI prompt data still used by replies.
- Retire the native-helper source, packages, and new-session protocol. Web QR is
  the recommended login, server-side Chrome is a fallback, and the extension is
  the remote-device import/renewal path.
...
- Reject invite-managed items in the manual-ship full-delivery path and point
  ...
  inventory source.
- 增加可见且在线时每 15 秒一次的有界仪表盘刷新与 `NumberFlow` 数字过渡。可信付款快照会在履约前更新普通订单金额、数量和下单时间；退款完成扣除金额，退款撤销恢复金额。
